"""Storage for notes, drafts, assessments and runs. All DB access goes through here.

SQLite locally and in tests; Postgres (e.g. Supabase) when DATABASE_URL is set, which
serverless hosting needs because its file system is temporary. Queries are written once
with `?` placeholders and are always parameterised. Timestamps are ISO-8601 UTC text.
"""
...
MIGRATIONS: tuple[str, ...] = (
    ...
    """
    BEGIN;
    ALTER TABLE notes ADD COLUMN processing_until TEXT;
    PRAGMA user_version = 7;
    COMMIT;
    """,
)

# Postgres starts at the current schema in one step (it has no history to replay).
# Append-only like MIGRATIONS; the version lives in the schema_meta table.
PG_MIGRATIONS: tuple[str, ...] = (
    """
    CREATE TABLE notes (
        ...
        content            TEXT NOT NULL,
        content_type       TEXT NOT NULL DEFAULT 'text' CHECK (content_type IN ('text', 'voice', 'unsupported')),
        tg_file_id         TEXT,
        ...
        worth_developing   INTEGER,
        triage_reason      TEXT,
        angle              TEXT,
        news_keywords      TEXT,
        sender             TEXT,
        transcript_clarity TEXT,
        unclear_ratio      DOUBLE PRECISION,
        media_mime         TEXT,
    ...
    CREATE TABLE assessments (
        ...
        parameters_json      TEXT NOT NULL,
        hard_flags_json      TEXT NOT NULL,
        overall              DOUBLE PRECISION NOT NULL CHECK (overall BETWEEN 0 AND 10),
        decision             TEXT NOT NULL CHECK (decision IN ('qualified', 'rejected', 'human_review')),
        scorecard_message_id BIGINT,
...
# Serialises concurrent cold starts migrating the same database.
PG_MIGRATION_LOCK = 72_431_907
...
def init_db(path: Path | None = None, database_url: str | None = None) -> None:
    """Choose the backend and apply pending migrations.

    An explicit path means SQLite (tests, local); otherwise DATABASE_URL means Postgres,
    and the configured DB_PATH is the SQLite fallback.
    """
    global _db_path, _pg_url
    url = database_url or (config.settings.database_url if path is None else None)
    if url:
        _db_path, _pg_url = None, url
        _migrate_postgres()
        return
    _pg_url = None
...
def _migrate_postgres() -> None:
    with _connect() as conn:
        conn.execute("SELECT pg_advisory_xact_lock(?)", (PG_MIGRATION_LOCK,))
        conn.execute("CREATE TABLE IF NOT EXISTS schema_meta (version INTEGER NOT NULL)")
        row = conn.execute("SELECT version FROM schema_meta").fetchone()
        version = row["version"] if row else 0
        for number, script in enumerate(PG_MIGRATIONS[version:], start=version + 1):
            conn.execute(script)
            version = number
...
def schema_version() -> int:
    with _connect() as conn:
        if _pg_url:
            return conn.execute("SELECT version FROM schema_meta").fetchone()["version"]
        return conn.execute("PRAGMA user_version").fetchone()[0]


class _PostgresConnection:
    """Lets the same `?`-style SQL run on psycopg (which uses %s placeholders)."""

    def __init__(self, conn: psycopg.Connection) -> None:
        self._conn = conn

    def execute(self, sql: str, params: tuple[Any, ...] = ()) -> psycopg.Cursor:
        return self._conn.execute(sql.replace("?", "%s"), params)


@contextmanager
def _connect() -> Iterator[Any]:
    """One connection per unit of work; commits on success, rolls back on exception."""
    if _pg_url:
        # prepare_threshold=None: Supabase's transaction pooler can't hold prepared statements.
        with psycopg.connect(_pg_url, row_factory=dict_row, prepare_threshold=None,
...
def claim_note(note_id: int, lease_seconds: int) -> bool:
    """Take exclusive processing of a note for a while; False if another worker holds it.

    Serverless hosts run several copies at once (a webhook retry plus the cron), so an
    in-process lock isn't enough. An expired lease can be re-claimed after a crash.
    """
    ...
    with _connect() as conn:
        cur = conn.execute(
            """
            UPDATE notes SET processing_until = ?
            WHERE id = ? AND status = 'new' AND (processing_until IS NULL OR processing_until < ?)
            """,
            (until, note_id, now.isoformat()),
        )
    return cur.rowcount == 1


def release_note(note_id: int) -> None:
    with _connect() as conn:
        conn.execute("UPDATE notes SET processing_until = NULL WHERE id = ?", (note_id,))
...
def count_revisions(note_id: int) -> int:
    with _connect() as conn:
        return conn.execute("SELECT COUNT(*) AS n FROM drafts WHERE note_id = ?", (note_id,)).fetchone()["n"]
